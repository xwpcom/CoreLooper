...
static int NextProtoSelectCallback(SSL* ssl, uint8_t** out, uint8_t* outlen,
  ...
  *outlen = (uint8_t)config->select_next_proto.size();
  return SSL_TLSEXT_ERR_OK;
}
...
static int AlpnSelectCallback(SSL* ssl, const uint8_t** out, uint8_t* outlen,
  ...
  *outlen = (uint8_t)config->select_alpn.size();
  return SSL_TLSEXT_ERR_OK;
}
    ...
    if (!config->shim_shuts_down) {
      for (;;) {
...

		{
			...
			auto ret=SSL_write(ssl.get(), req.c_str(), req.length());
			DV("SSL_Write ret=%d", ret);
			x = 1;
		}
...
static int Main(int argc, char **argv) {
  ...
  for (int i = 0; i < config.resume_count + 1; i++) {
    ...
    if (!DoExchange(&session, ssl_ctx.get(), &config, is_resume,offer_session.get()))
	{
...
int CRYPT_EXT_CLASS openssl_main(int argc, char **argv)
{
