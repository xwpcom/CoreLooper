 ...
 */
static int asn1_get_len_EX( unsigned char **p,
                         unsigned char *end,
                         int *len )
{
    if( ( end - *p ) < 1 )
                         ...
                         int *len, int tag )
{
    ...
    return( asn1_get_len_EX( p, end, len ) );
}
                         ...
                         mpi_EX *X )
{
    ...
    ret = mpi_read_binary_EX( X, *p, len );
                            ...
                            x509_buf_EX *serial )
{
    ...
    if( ( ret = asn1_get_len_EX( p, end, &serial->len ) ) != 0 )
                          ...
                          x509_name_EX *cur )
{
    ...
    *p += oid->len;
    ...
    if( ( ret = asn1_get_len_EX( p, end, &val->len ) ) != 0 )
        return( POLARSSL_ERR_X509_CERT_INVALID_NAME | ret );
...
{
    int ret, entry_len;
    x509_crl_entry_EX *cur_entry = entry;
    ...
    while( *p < end )
    {
        ...
        if ( *p < end ) {
            cur_entry->next = (x509_crl_entry_EX*)malloc( sizeof( x509_crl_entry_EX ) );
            cur_entry = cur_entry->next;
            memset( cur_entry, 0, sizeof( x509_crl_entry_EX ) );
        }
    }
...
int x509parse_crt_EX( x509_cert_EX *chain, unsigned char *buf, int buflen )
{
    ...
    {
        crt->next = (x509_cert_EX *) malloc( sizeof( x509_cert_EX ) );
        ...
        crt = crt->next;
        memset( crt, 0, sizeof( x509_cert_EX ) );
    }
    ...
    if( s1 != NULL )
    {
         ...
         */
        len = 0;
        ret = base64_decode_EX( NULL, &len, s1, s2 - s1 );
        ...
        if( ( ret = base64_decode_EX( p, &len, s1, s2 - s1 ) ) != 0 )
        {
            free( p );
            return( POLARSSL_ERR_X509_CERT_INVALID_PEM | ret );
        }
    ...
    if( ( ret = rsa_check_pubkey_EX( &crt->rsa ) ) != 0 )
    {
        x509_free_EX( crt );
        return( ret );
    }

    crt->rsa.len = mpi_size_EX( &crt->rsa.N );
    ...
    if( buflen > 0 )
    {
        ...
        return( x509parse_crt_EX( crt, buf, buflen ) );
    }
...
int x509parse_crl_EX( x509_crl_EX *chain, unsigned char *buf, int buflen )
{
    ...
    {
        crl->next = (x509_crl_EX *) malloc( sizeof( x509_crl_EX ) );

        if( crl->next == NULL )
        {
            x509_crl_free_EX( crl );
            return( 1 );
        }

        crl = crl->next;
        memset( crl, 0, sizeof( x509_crl_EX ) );
    }
    ...
    if( buflen > 0 )
    {
        ...
        return( x509parse_crl_EX( crl, buf, buflen ) );
    }
...
int x509parse_crtfile_EX( x509_cert_EX *chain, char *path )
{
    ...
    ret = x509parse_crt_EX( chain, buf, (int) n );
...
int x509parse_crlfile_EX( x509_crl_EX *chain, char *path )
{
    ...
    ret = x509parse_crl_EX( chain, buf, (int) n );
 ...
 */
int x509parse_key_EX( rsa_context_EX *rsa, unsigned char *buf, int buflen,
                                     unsigned char *pwd, int pwdlen )
{
    int ret, len, enc;
    unsigned char *s1, *s2;
    ...
    if( s1 != NULL )
    {
        ...
        if( ( ret = base64_decode_EX( buf, &len, s1, s2 - s1 ) ) != 0 )
        {
            free( buf );
            return( ret | POLARSSL_ERR_X509_KEY_INVALID_PEM );
        }
    ...
    memset( rsa, 0, sizeof( rsa_context_EX ) );
    ...
    if( ( ret = rsa_check_privkey_EX( rsa ) ) != 0 )
    {
        if( s1 != NULL )
            free( buf );
 ...
 */
int x509parse_keyfile_EX( rsa_context_EX *rsa, char *path, char *pwd )
{
    int ret;
    size_t n;
    unsigned char *buf;
    ...
    if( pwd == NULL )
        ret = x509parse_key_EX( rsa, buf, (int) n, NULL, 0 );
    else
        ret = x509parse_key_EX( rsa, buf, (int) n,
...
int x509parse_cert_info_EX( char *buf, size_t size, char *prefix, x509_cert_EX *crt )
{
    ...
    ret = snprintf( p, n, "\n%sissuer name   : ", prefix );
    SAFE_SNPRINTF();
    ret = x509parse_dn_gets_EX( p, n, &crt->issuer  );
    SAFE_SNPRINTF();

    ret = snprintf( p, n, "\n%ssubject name  : ", prefix );
    SAFE_SNPRINTF();
    ret = x509parse_dn_gets_EX( p, n, &crt->subject );
    SAFE_SNPRINTF();
...
int x509parse_crl_info_EX( char *buf, size_t size, char *prefix, x509_crl_EX *crl )
{
    ...
    ret = snprintf( p, n, "\n%sissuer name   : ", prefix );
    SAFE_SNPRINTF();
    ret = x509parse_dn_gets_EX( p, n, &crl->issuer );
    SAFE_SNPRINTF();
...
/*
 * Return 0 if the x509_time_EX is still valid, or 1 otherwise.
 */
int x509parse_time_expired_EX( x509_time_EX *to )
{
    struct tm *lt;
    time_t tt;
 ...
 */
int x509parse_revoked( x509_cert_EX *crt, x509_crl_EX *crl )
{
    x509_crl_entry_EX *cur = &crl->entry;

    while( cur != NULL && cur->serial.len != 0 )
    {
        if( memcmp( crt->serial.p, cur->serial.p, crt->serial.len ) == 0 )
        {
            if( x509parse_time_expired_EX( &cur->revocation_date ) )
                return( 1 );
        }
                       ...
                       unsigned char *out )
{
    switch( alg )
    {
        ...
        case SIG_RSA_SHA1   : sha1_EX( in, len, out ); break;
#endif
                      ...
                      char *cn, int *flags )
{
    ...
    *flags = 0;
    ...
    if( x509parse_time_expired_EX( &crt->valid_to ) )
    ...
    *flags |= BADCERT_NOT_TRUSTED;
    ...
    while( cur != NULL && cur->version != 0 )
    {
        ...
        if( rsa_pkcs1_verify_EX( &cur->rsa, RSA_PUBLIC, hash_id,
                              0, hash, crt->sig.p ) != 0 )
    ...
    while( trust_ca != NULL && trust_ca->version != 0 )
    {
        ...
        if( rsa_pkcs1_verify_EX( &trust_ca->rsa, RSA_PUBLIC, hash_id,
                              0, hash, crt->sig.p ) == 0 )
        {
            /*
    ...
    while( trust_ca != NULL && ca_crl != NULL && ca_crl->version != 0 )
    {
         ...
         */
        if( x509parse_time_expired_EX( &ca_crl->next_update ) )
...
{
    x509_cert_EX *cert_cur = crt;
    x509_cert_EX *cert_prv;
    x509_name_EX *name_cur;
    x509_name_EX *name_prv;
    ...
    do
    {
        ...
        while( name_cur != NULL )
        {
            ...
            memset( name_prv, 0, sizeof( x509_name_EX ) );
            free( name_prv );
        }
    ...
    do
    {
        ...
        memset( cert_prv, 0, sizeof( x509_cert_EX ) );
        if( cert_prv != crt )
            free( cert_prv );
    }
    while( cert_cur != NULL );
...
{
    x509_crl_EX *crl_cur = crl;
    x509_crl_EX *crl_prv;
    x509_name_EX *name_cur;
    x509_name_EX *name_prv;
    x509_crl_entry_EX *entry_cur;
    x509_crl_entry_EX *entry_prv;
    ...
    do
    {
        ...
        while( entry_cur != NULL )
        {
            ...
            memset( entry_prv, 0, sizeof( x509_crl_entry_EX ) );
            free( entry_prv );
        }
    ...
    do
    {
        ...
        memset( crl_prv, 0, sizeof( x509_crl_EX ) );
        if( crl_prv != crl )
            free( crl_prv );
    }
    while( crl_cur != NULL );
