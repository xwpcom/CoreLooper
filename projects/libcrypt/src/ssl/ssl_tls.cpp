                     ...
                     unsigned char *dstbuf, int dlen )
{
     ...
     */
    sha1_hmac_EX( S2, hs, tmp + 20, nb, tmp );

    for( i = 0; i < dlen; i += 20 )
    {
        sha1_hmac_EX( S2, hs, tmp, 20 + nb, h_i );
        sha1_hmac_EX( S2, hs, tmp, 20,      tmp );
...
int ssl_derive_keys( ssl_context *ssl )
{
    ...
    if( ssl->resume == 0 )
    {
        ...
        if( ssl->minor_ver == SSL_MINOR_VERSION_0 )
        {
            for( i = 0; i < 3; i++ )
            {
                ...
                sha1_starts_EX( &sha1_EX );
                sha1_update_EX( &sha1_EX, padding, 1 + i );
                sha1_update_EX( &sha1_EX, ssl->premaster, len );
                sha1_update_EX( &sha1_EX, ssl->randbytes,  64 );
                sha1_finish_EX( &sha1_EX, sha1sum );
    ...
    if( ssl->minor_ver == SSL_MINOR_VERSION_0 )
    {
        for( i = 0; i < 16; i++ )
        {
            ...
            sha1_starts_EX( &sha1_EX );
            sha1_update_EX( &sha1_EX, padding, 1 + i );
            sha1_update_EX( &sha1_EX, ssl->session->master, 48 );
            sha1_update_EX( &sha1_EX, ssl->randbytes, 64 );
            sha1_finish_EX( &sha1_EX, sha1sum );
        ...
        memset( &sha1_EX, 0, sizeof( sha1_EX ) );
    ...
    switch( ssl->session->cipher )
    {
        ...
        case SSL_RSA_RC4_128_MD5:
        case SSL_RSA_RC4_128_SHA:
            arc4_setup_EX( (arc4_context_EX *) ssl->ctx_enc, key1, ssl->keylen );
            arc4_setup_EX( (arc4_context_EX *) ssl->ctx_dec, key2, ssl->keylen );
            break;
#endif
...
void ssl_calc_verify( ssl_context *ssl, unsigned char hash[36] )
{
    ...
    memcpy( &sha1_EX, &ssl->fin_sha1, sizeof( sha1_context ) );
    ...
    if( ssl->minor_ver == SSL_MINOR_VERSION_0 )
    {
        ...
        sha1_update_EX( &sha1_EX, ssl->session->master, 48 );
        sha1_update_EX( &sha1_EX, pad_1, 40 );
        sha1_finish_EX( &sha1_EX, hash + 16 );

        sha1_starts_EX( &sha1_EX );
        sha1_update_EX( &sha1_EX, ssl->session->master, 48 );
        sha1_update_EX( &sha1_EX, pad_2, 40 );
        sha1_update_EX( &sha1_EX, hash + 16, 20 );
        sha1_finish_EX( &sha1_EX, hash + 16 );
    }
    else /* TLSv1 */
    {
                          ...
                          unsigned char *ctr, int type )
{
    ...
    sha1_starts_EX( &sha1_EX );
    sha1_update_EX( &sha1_EX, secret,  20 );
    sha1_update_EX( &sha1_EX, padding, 40 );
    sha1_update_EX( &sha1_EX, header,  11 );
    sha1_update_EX( &sha1_EX, buf,  len );
    sha1_finish_EX( &sha1_EX, buf + len );

    memset( padding, 0x5C, 40 );
    sha1_starts_EX( &sha1_EX );
    sha1_update_EX( &sha1_EX, secret,  20 );
    sha1_update_EX( &sha1_EX, padding, 40 );
    sha1_update_EX( &sha1_EX, buf + len, 20 );
    sha1_finish_EX( &sha1_EX, buf + len );
}
...
static int ssl_encrypt_buf( ssl_context *ssl )
{
    ...
    if( ssl->ivlen == 0 )
    {
        ...
        arc4_crypt_EX( (arc4_context_EX *) ssl->ctx_enc,
                    ssl->out_msg, ssl->out_msglen );
#else
        return( POLARSSL_ERR_SSL_FEATURE_UNAVAILABLE );
#endif
...
#if defined(POLARSSL_ARC4_C)
        padlen = 0;
        arc4_crypt_EX( (arc4_context_EX *) ssl->ctx_dec,
...
int ssl_write_record( ssl_context *ssl )
{
    ...
    if( ssl->out_msgtype == SSL_MSG_HANDSHAKE )
    {
        ...
        sha1_update_EX( &ssl->fin_sha1, ssl->out_msg, len );
    }
...
int ssl_read_record( ssl_context *ssl )
{
        ...
        ssl->in_hslen < ssl->in_msglen )
    {
        ...
        sha1_update_EX( &ssl->fin_sha1, ssl->in_msg, ssl->in_hslen );
...
int ssl_parse_certificate( ssl_context *ssl )
{
    ...
    if( ( ssl->peer_cert = (x509_cert_EX *) malloc(
                    sizeof( x509_cert_EX ) ) ) == NULL )
    {
    ...
    memset( ssl->peer_cert, 0, sizeof( x509_cert_EX ) );
    ...
    while( i < ssl->in_hslen )
    {
        ...
        ret = x509parse_crt_EX( ssl->peer_cert, ssl->in_msg + i, n );
        if( ret != 0 )
        {
            SSL_DEBUG_RET( 1, " x509parse_crt_EX", ret );
            return( ret );
        }
    ...
    if( ssl->authmode != SSL_VERIFY_NONE )
    {
        ...
        ret = x509parse_verify_EX( ssl->peer_cert, ssl->ca_chain, ssl->ca_crl,
...
static void ssl_calc_finished(
                ...
                md5_context *md5, sha1_context *sha1_EX )
{
    int len = 12;
    char *sender;
    ...
    SSL_DEBUG_BUF( 4, "finished sha1_EX state", (unsigned char *)
                   sha1_EX->state, sizeof( sha1_EX->state ) );
    ...
    if( ssl->minor_ver == SSL_MINOR_VERSION_0 )
    {
        ...
        sha1_update_EX( sha1_EX, (unsigned char *) sender, 4 );
        sha1_update_EX( sha1_EX, ssl->session->master, 48 );
        sha1_update_EX( sha1_EX, padbuf, 40 );
        sha1_finish_EX( sha1_EX, sha1sum );
        ...
        sha1_update_EX( sha1_EX, ssl->session->master, 48 );
        sha1_update_EX( sha1_EX, padbuf , 40 );
        sha1_update_EX( sha1_EX, sha1sum, 20 );
        sha1_finish_EX( sha1_EX, buf + 16 );
    ...
    else
    {
        ...
        sha1_finish_EX( sha1_EX, padbuf + 16 );
    ...
    memset( sha1_EX, 0, sizeof( sha1_context ) );
...
int ssl_parse_finished( ssl_context *ssl )
{
    ...
    ssl_calc_finished( ssl, buf, ssl->endpoint ^ 1, &md5, &sha1_EX );
...
void ssl_set_ca_chain( ssl_context *ssl, x509_cert_EX *ca_chain,
                       x509_crl_EX *ca_crl, char *peer_cn )
{
...
void ssl_set_own_cert( ssl_context *ssl, x509_cert_EX *own_cert,
                       rsa_context_EX *rsa_key )
{
...
const char *ssl_get_cipher( ssl_context *ssl )
{
#ifdef _DEBUG
	switch( ssl->session->cipher )
    {
